...
##No se toma el valor 1 como valor posible a generar
def generador_aleatorio_lineal(x, k, g, c):
    a = 1 + 4 * k
    m = 2 ** g
    xi = func_aleatoria_lineal(x, a, m, c)
    ri = truncate((xi / (m)),4)                     ## los resultados obtenidos son diferentes a los ejercicios propuestos en clase, ya que dividimos por m y no m-1.
    return (a*x)+c,xi, ri
...
def prueba_chi2(numeros, cant_int):
    ...
    for i in range(len(numeros)):           ##Para cada numero analiza el intervalo en el cual se encuentra y acumula 1 al valor. Aqui vemos la frecuencia obtenida.
    ...
    sumatoria = []
    suma = 0
    for i in range(cant_int):               ##Debido a que los numeros generados tienen demasiados decimales, con este método truncamos a 4 todos los obtenidos.
...
def metodo_lineal():
    ...
    print(res)
    i = 21                                                                                                    ## Se utiliza para ir agregando al vector de numeros un nuevo elemento
    prox = int(input("Ingrese 1 si quiere agregar un numero a la serie y 0 si desea terminar: "))             ## Permite el ingreso de un valor por vez.
    while prox != 0:
...
def op2():
    ...
    if (cant/intervalos) < 5:                                                                   ##Validamos que las frecuencias esperadas de cada intervalo sean mayores a 5, de lo contrario no se puede realizar la prueba
        print("Error al cargar datos, los intervalos tienen frecuencia esperada menor a 5\n")
    else:
        numeros = [random.uniform(0, 1) for x in range(cant)]                               ##Random.Uniform no abarca el 1. En esta sentencia creamos x cantidad de numeros y lo agregamos a un vector
