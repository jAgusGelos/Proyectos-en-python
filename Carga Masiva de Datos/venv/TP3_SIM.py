...
def esperado_exponencial(x,l, u):
    if l== 0:
        l = 1/u
...
def obtener_esperados_exponencial(numeros, intervalos, l, u):
    for i in range(len(intervalos)-1):
        x = esperado_exponencial(intervalos[i],l,u)
        y = esperado_exponencial(intervalos[i+1], l, u)
        esperado = y-x
        frec_esperada.append(truncate((esperado * len(numeros)), 2))
    return frec_esperada
...
def prueba_chi2(numeros, cant_int, ddof, l, u):
    ...
    if ddof == 2:
        ...
        if es_entero:
            #poisson
            frec_esperada = obtener_esperados_poisson(numeros,intervals,l)
        else:
            #expon
            frec_esperada = obtener_esperados_exponencial(numeros,intervals,l,u)
    elif ddof == 0:
...
def poisson(numeros, cant_int, ddof, l, u):

    #Obtenemos la frecuencia esperada
    contador = len(np.unique(numeros))
    frec_esperada = obtener_esperados_poisson(numeros, intervals, l)


    for i in range(numeros):
        contador[numeros[i]-1] += 1
